...
def correct_spelling(text, preserve_medical_terms=True):
    ...
    protected_medical_terms = [
        ...
        'polydipsia', 'hemoptysis', 'hematemesis', 'melena', 'hematuria' , 'cold'
    ]
...
def display_prediction_results(results):
    """Display prediction results with simplified confidence display"""
    ...
    st.markdown(f"**🔍 {results['predicted_disease']}**")
    
    # Display top 3 predictions with labels
    if results['top_3_predictions']:
        st.markdown("**Alternative Possibilities:**")
        labels = ["Most Likely", "Likely", "Maybe"]
        for i, (disease, conf) in enumerate(results['top_3_predictions'][:3], 0):
            label = labels[i] if i < len(labels) else f"Option {i+1}"
            st.write(f"**{label}:** {disease}")

# --- Simplified Google Sheets and CSV integration ---
def save_feedback_data(data_row):
    """Simple function to save feedback to Google Sheets and local CSV"""
    ...
    try:
        ...
        # Check if headers exist, add if needed
        ...
        st.warning("⚠️ Could not save to Google Sheets, using local storage...")
    ...
    try:
        ...
        st.success("✅ Feedback saved to local file!")
        return True
...
def main():
    ...
    if not st.session_state.id_applied:
        ...
        pid_input = st.text_input("Participant ID (format: a001, a002, etc.)")

        if st.button("Apply"):
            if pid_input and re.fullmatch(r"a\d{3}", pid_input.strip()):
